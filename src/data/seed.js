...
// ── БАЗА ЗНАНИЙ ──────────────────────────────────────────────────────────
export const KB_CATEGORIES = ['Начало работы', 'Воронки и сделки', 'Документы', 'Задачи', 'Реестры', 'Команда', 'Интеграции']

const st = (...texts) => texts.map((t, i) => ({ id: `s${i}`, text: t, image: null }))
  ...
  return [
    {
      ...
      steps: st(
        ...
        'Внизу слева — текущий пользователь. Нажмите на него, чтобы переключиться (демо разных ролей).',
        'Все данные сохраняются в браузере. Кнопка «Сбросить демо» внизу возвращает исходные данные.',
        'Начните с раздела «Воронки» — это сердце системы, где ведутся все сделки.',
      ),
    },
    {
      id: 'kb-funnels', category: 'Воронки и сделки', emoji: '🗂️', title: 'Воронки и стадии', updatedAt: '', illo: 'kanban',
      intro: 'Сделки разложены по 4 воронкам: Лиды, Физ лица, ЮЛ·РЦТО, ТО·Туры. Каждая воронка — это канбан-доска со стадиями.',
      steps: st(
        'Откройте раздел «Воронки». Вверху — переключатель воронок с числом сделок и суммой.',
        'Сделки показаны карточками в колонках-стадиях. Перетащите карточку мышью в другую стадию, чтобы сменить этап.',
        'Наведите на значок «i» под названием стадии — там подсказка-требование по этому этапу.',
        'Поиск вверху ищет по названию, телефону, ребёнку, компании.',
        'Нажмите на карточку — откроется полная карточка сделки.',
      ),
    },
    {
      id: 'kb-deal', category: 'Воронки и сделки', emoji: '📇', title: 'Карточка сделки', updatedAt: '', illo: 'deal',
      intro: 'Карточка устроена как в современных CRM: стадии сверху, поля слева, лента событий и комментарии справа.',
      steps: st(
        'Вверху — переключатель стадий «шевронами». Клик по стадии перемещает сделку (изменение запишется в ленту).',
        'Кнопки «Позвонить» и «1С» — быстрые действия (в демо имитируются и пишутся в ленту).',
        'Слева — блок «Дела», поля сделки и участники. Любое поле редактируется по клику.',
        'Справа — лента: автоматически фиксируются смена стадии, правки, документы, звонки.',
        'Внизу справа — поле комментария (Ctrl+Enter — отправить).',
      ),
    },
    {
      id: 'kb-members', category: 'Воронки и сделки', emoji: '👨‍👩‍👧', title: 'Участники, родители и дети', updatedAt: '', illo: 'deal',
      intro: 'Сделка — корневой элемент. Внутри неё — участники (родители/плательщики), у каждого свой договор, контакты и дети.',
      steps: st(
        'В карточке сделки нажмите «+ Родитель» (или «+ Плательщик» в ЮЛ/ТО), чтобы добавить участника.',
        'У участника задайте № договора, сумму, оплату и статус договора (Нет → … → Подписан).',
        'В блоке «Контакты» добавьте маму и папу (разные телефоны) — «+ новый» или «+ привязать» из реестра.',
        'В блоке «Дети» нажмите «+ добавить», затем «Изменить» — заполните ФИО, дату рождения, свидетельство, лагерь и смену.',
        'Сумма сделки считается автоматически из сумм всех участников.',
      ),
    },
    {
      id: 'kb-builder', category: 'Документы', emoji: '🛠️', title: 'Конструктор документов (без Word)', updatedAt: '', illo: 'docbuilder',
      intro: 'Самый простой способ создать шаблон документа — прямо в приложении, без Word.',
      steps: st(
        'Раздел «Шаблоны» → кнопка «+ Создать в редакторе».',
        'Слева пишите текст документа. Справа — каталог полей: нажмите на поле, и оно вставится в текст в месте курсора.',
        'Для списка детей нажмите «Вставить весь список» — добавится блок, который повторится для каждого ребёнка.',
        'Вкладка «Предпросмотр» показывает готовый документ на примере реальной сделки.',
        'Выберите воронку и «Для кого» (по участнику / на всю сделку), задайте название и нажмите «Сохранить».',
      ),
    },
    {
      ...
      steps: st(
        ...
        'Вставьте метки в свой .docx в нужных местах. Для списка детей оберните строку в {#дети} … {/дети}.',
        'Нажмите «Загрузить Word» и выберите файл — система найдёт метки.',
        'В окне настройки сопоставьте каждую метку с полем (обычно подставляется автоматически).',
        'Проверьте «Предпросмотр» справа и сохраните. Кнопка «↓ Пример» скачивает готовый образец.',
      ),
    },
    {
      ...
      steps: st(
        ...
        'Выберите шаблон (и участника, если документ по участнику).',
        'Нажмите «Сформировать .docx» — файл скачается заполненным.',
        'Запись появится в разделе «Документы» со статусом подписания, который можно менять.',
        'В реестре «Документы» документ можно скачать повторно кнопкой «↓».',
      ),
    },
    {
      id: 'kb-tasks', category: 'Задачи', emoji: '✅', title: 'Дела и задачи', updatedAt: '', illo: 'tasks',
      intro: 'Простые дела с тегами и сроком — как в amoCRM. Ставятся из сделки и собираются в разделе «Задачи».',
      steps: st(
        'В карточке сделки в блоке «Дела» впишите задачу, выберите тег (Звонок, Оплата, Документы…), дату и время.',
        'Нажмите «Поставить». Дело привяжется к сделке, постановка запишется в ленту.',
        'Отметьте дело галочкой, когда выполнено — оно зачеркнётся, а в ленту попадёт «Дело выполнено».',
        'Раздел «Задачи» в меню показывает все дела по срокам: Просрочено / Сегодня / Завтра / Позже.',
        'Переключатель «Мои / Все» фильтрует по ответственному.',
      ),
    },
    {
      ...
      steps: st(
        ...
        '«Компании» — заказчики, наше ЮЛ, партнёры (с ИНН, КПП, адресом).',
        '«Лагеря и смены» — объекты (лагеря, санатории) и их смены с датами, ценой и загрузкой.',
        'Слева выбираете объект, справа — список смен; «+ Смена» добавляет новую.',
        'Эти данные используются при заполнении детей в сделке и в документах.',
      ),
    },
    {
      ...
      steps: st(
        ...
        '«+ Создать канал» — групповой чат для команды.',
        '«Пользователи» (только для администратора) — список сотрудников и их роли.',
        'Роли: Администратор (всё), Менеджер (правит сделки и реестры), Наблюдатель (только просмотр).',
        'Кнопка «Войти как» и переключатель внизу слева позволяют посмотреть систему глазами другой роли.',
      ),
    },
    {
      ...
      steps: st(
        ...
        'Телефония: входящий звонок создаёт лид и поднимает карточку, есть запись разговоров и звонок из карточки.',
        '1С: двусторонний обмен — из CRM уходит контрагент, договор и счёт; обратно прилетает статус оплаты.',
        'Подробности и список сервисов — по клику на «Телефония»/«1С» вверху раздела «Воронки».',
        'Боевые интеграции включаются после переноса приложения на сервер (VPS).',
      ),
    },
  ]
}
